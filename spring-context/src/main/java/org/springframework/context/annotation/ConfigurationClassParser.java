 ...
 * 解析一个@Configuration配置类定义, 填充ConfigurationClass对象(解析单个配置类可能导致任意数量的ConfigurationClass对象，因为一个配置类可以导入另一个使用@Import注解)
 *
 ...
 * 这种基于ASM的实现避免了反射和立即加载, 以便在Spring ApplicationContext中有效地与延迟类加载交互操作
 * @author Chris Beams
